...
export const FetchLocaleCurrency = async (locale: string) => {
   let userLocale = locale;
   let UserCurencyData = await CacheStorage.get("UserCurrency", `./locales/${userLocale}/currency.json`) as Currency | undefined;

   if (UserCurencyData == undefined) {
      try {
         // fetches data from the public folder.
         console.log("Fetching user locale data from the web...");
         UserCurencyData = await (await fetch(`./locales/${userLocale}/currency.json`)).json() as Currency;
         await CacheStorage.add("UserCurrency", `./locales/${userLocale}/currency.json`, UserCurencyData, 60 * 60 * 24 * 30);
